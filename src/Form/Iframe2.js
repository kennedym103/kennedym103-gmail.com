...
class Iframe2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      bannerSpeed: '2000',
  ...
  componentWillReceiveProps(state) {
    if (state.form.bannerGeneratorForm.values != undefined) {
      ...
      this.setState({
        ...
        buttonLineHeight: currentState.buttonLineHeight,
  ...
  <a href=${this.state.url} target="_blank" class="liveframe bannerContainer" style="width: ${this.state.width}; height: ${
    ...
    }; background: ${this.state.colorButtonBackground}; font-family: ${
      this.state.font
    };"  id="bg-exit2">${this.state.buttonText}</a>
      <div class="banner-messages" style="background-color: ${this.state.colorMessage};">

            <div class="banner-message banner-message-2" style="display: block; opacity: 1; color: ${
              this.state.color2
...
export default connect(mapStateToProps)(Iframe2);
