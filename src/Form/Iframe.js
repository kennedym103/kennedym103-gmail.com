...
class Iframe extends Component {
  ...
  _updateIframe() {
    ...
    <a href=${this.state.url} target="_blank" class="liveframe bannerContainer" style="width: ${this.state.width}; height: ${
      ...
      <div class="banner-message banner-message-3" style="${slide3Visibile} ${slide5Visibile} color: ${
                this.state.color3
              }; font-size: ${this.state.fontSize3} !important; font-family: ${
          this.state.font
        }; ${uppercase3} line-height: ${this.state.lineHeight3}px; ${pos3}">${this.state.message3}</div>
  ...
  render(props) {
    ...
    return (
      ...
      <iframe ref="iframe" charSet="utf-8" style={iframeStyle} scrolling="no" />
        <div className="buttons__container_animate">
          <button onClick={(e) => this.setState({animate: true})}>Animate</button>
        </div>
      </div>
    );
  }
